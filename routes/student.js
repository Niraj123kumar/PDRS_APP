...
router.post('/schedule-defense', verifyToken, async (req, res) => {
    ...
    try {
        ...
        return res.json({ success: true, defenseDate: date, googleEventUrl });
    } catch (err) {
...
router.delete('/calendar-event', verifyToken, async (req, res) => {
    try {
...
router.get('/badges', verifyToken, (req, res) => {
    ...
    res.json({ earned, allBadges });
});

// GET /api/student/dashboard-insights
router.get('/dashboard-insights', verifyToken, (req, res) => {
...
router.get('/export-report', verifyToken, async (req, res) => {
    try {
